...
class UserController extends Controller
{
    ...
    public function indexAction()
    {
        ...
        if ($_centre_id == true && $_is_admin == true) {
            ...
            $_users = array_merge($_user_manager->getAllUsersByCodeCentreAndRole($_code_centre, RoleName::ID_ROLE_VISITE), $_user_manager->getAllUsersByCodeCentreAndRole($_code_centre, RoleName::ID_ROLE_RECEPTION));
    ...
    public function editAction(User $_user)
    {
        ...
        if (!$_is_super_admin && !$_centre_id && !$_is_same_user_as_connected) { // Not SUPER_ADMIN and Not ADMIN and Not same user connected
            ...
            $_liste_id = $_centre_manager->getIdListCentresAndSubCentres($_user->getCtCentre()->getCtrCode());
            /* if ($_centre_id !== $_user->getCtCentre()->getId()) { // ADMIN cannot edit User from another Centre */
            if(!$_is_admin && !in_array($_liste_id, $_centre_id)) {
                $_show_edit_page = false;
            } else {
