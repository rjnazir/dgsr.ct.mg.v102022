...
class UserManager
{
    ...
    /**
     * Recuperation des verificateurs dans un centre et ses souscentres (itinérantes)
     * @param $_code_centre
     * @return array()
     */
    public function getAllVerificateurByCentreAndSubCentres($_code_centre){
        $_entity_ctr = EntityName::CT_CENTRE;
        $_entity_usr = EntityName::USER;
        $_sql = "SELECT u FROM $_entity_usr u
                            WHERE u.ctCentre IN (SELECT c.id FROM $_entity_ctr c WHERE c.ctrCode = :ctr_code )
                            AND u.ctRole = :usr_role
                ORDER BY u.id ASC";
        $_query = $this->_entity_manager->createQuery($_sql);
        $_query->setParameter('ctr_code', $_code_centre);
        $_query->setParameter('usr_role', RoleName::ID_ROLE_VERIFICATEUR);
        $_result = $_query->getResult();
        return $_result;
    }

    /**
     * Récupération des utilisateurs dans un centre et ses sous-centres
     * @param $_code_centre
     * @param $_role
     * @return array()
     */
    public function getAllUsersByCodeCentreAndRole($_code_centre, $_role)
    {
        $_entity_ctr = EntityName::CT_CENTRE;
        $_entity_usr = EntityName::USER;
        $_sql = "SELECT u FROM $_entity_usr u
        ...
        $_query = $this->_entity_manager->createQuery($_sql);
        $_query->setParameter('ctr_code', $_code_centre);
        $_query->setParameter('usr_role', $_role);
        $_result = $_query->getResult();
        return $_result;
    }
    ...
    public function updateVerificateur($_user, $_user_connected) {
        // Traitement centre
        if ($_user_connected->getCtRole()->getId() == RoleName::ID_ROLE_SUPERADMIN and 
            $_user_connected->getCtRole()->getId() == RoleName::ID_ROLE_ADMIN) {
            $_user_centre = $_user_connected->getCtCentre();
            $_user->setCtCentre($_user_centre);
        }
