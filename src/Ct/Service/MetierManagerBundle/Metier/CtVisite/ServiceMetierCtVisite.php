...
//        $_usage_tarif            = $_usage_tarif_manager->getRepository()->findByCtUsage($_usage->getId());
    ...
    /**
     * Vérification si la précédente visite est inapte
    ...
    /**
     *  Récupération l'avant dernière visite inapte
     *  @param \DateTime $_visite_date
     *  @param integer $_ct_carte_grise_id
     *  @return array $_resultat
     */
    function getCtVisiteInapteBeforeContre($_visite_date, $_ct_carte_grise_id)
    {
        $_visite = EntityName::CT_VISITE;
        $_dql = "SELECT vst FROM $_visite vst
                 WHERE vst.vstCreated < :date_visite
                 AND vst.ctCarteGrise = :ct_carte_grise
                 AND vst.vstIsContreVisite = :is_contre_visite
                 ORDER BY vst.vstCreated DESC";
        $_query = $this->_entity_manager->createQuery($_dql);
        $_query->setParameter('date_visite', $_visite_date);
        $_query->setParameter('ct_carte_grise', $_ct_carte_grise_id);
        $_query->setParameter('is_contre_visite', 0);
        $_query->setMaxResults(1);
        $_result = $_query->getResult();
        foreach($_result as $_result)
        {
            $_resultat = new \stdClass();
            $_resultat->NUM_PV = $_result->getId(); /* $_result->getVstNumPv(); */
            $_resultat->NOM_CENTRE = $_result->getCtCentre()->getCtrNom();
            $_resultat->DATE_VISITE = $_result->getVstCreated()->format('d/m/Y');
        }
        return $_resultat;
    }
     ...
     */
    public function getAllVisiteByCentreAndDateNotInUIT($_centre_id)
    {
        $_date = new \DateTime();
