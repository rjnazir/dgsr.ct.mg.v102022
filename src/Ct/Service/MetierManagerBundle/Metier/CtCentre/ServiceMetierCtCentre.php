...
class ServiceMetierCtCentre
{
    ...
    /**
     * Récuperer tous les centres avec condtions
     * @param array $_condition
     * @param array $_order
     * @return array
     */
    public function getCentresWithConditions($_condition, $_order){
        return $this->getRepository()->findBy($_condition, $_order);
    }
    ...
    /**
     * Récupérer la liste des ID d'un centre et ces sous-centres
     * @param $_code_centre : Code du centre
     * @return array
     */
    public function getIdListCentresAndSubCentres($_code_centre)
    {
        $_liste = array(); $_i = 0;
        $_liste_centres = $this->getCentresWithConditions(array('ctrCode' => $_code_centre), array('id' => 'ASC'));
        foreach($_liste_centres as $_centre){
            $_liste[$_i] = $_centre->getId();
            $_i++;
        }
        return $_liste; 
    }
